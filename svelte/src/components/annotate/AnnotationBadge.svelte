<script lang="ts">
  ...
  let { index, rect, clickOffset, color, onClick, variant = "numbered" }: Props = $props();

  const isMulti = $derived(variant === "multi-pending");
  const isPending = $derived(variant === "pending" || isMulti);
  const half = $derived(isMulti ? 13 : 11);
  let hovered = $state(false);
</script>

<div
  class="rm-badge-wrap{isMulti ? ' rm-badge-wrap--multi' : ''}"
  style:top="{rect.top + clickOffset.y - half}px"
  style:left="{rect.left + clickOffset.x - half}px"
>
  <button
    class="rm-badge{isMulti ? ' rm-badge--multi' : ''}"
